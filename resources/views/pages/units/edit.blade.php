        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-slate-200">
                <div class="px-8 py-6 border-b border-slate-100 bg-slate-50/50">
                    <h3 class="text-xl font-bold text-slate-800">Edit Informasi Unit</h3>
                    <p class="text-sm text-slate-500 mt-1">Perbarui nama, kode, atau status unit kerja.</p>
                </div>

                <form action="{{ route('unit.update', $unit->id) }}" method="POST" class="p-8">
                    @csrf
                    @method('PUT')
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <div class="col-span-2">
                            ...
                            <input type="text" name="name" class="w-full border-slate-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 transition-colors @error('name') border-rose-500 @enderror" value="{{ old('name', $unit->name) }}" required>
                        ...
                        <div>
                            ...
                            <input type="text" name="code" class="w-full border-slate-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 uppercase transition-colors @error('code') border-rose-500 @enderror" value="{{ old('code', $unit->code) }}">
