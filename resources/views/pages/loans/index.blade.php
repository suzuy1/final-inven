<x-app-layout>
    ...
    <div class="py-8 bg-gray-50 min-h-screen font-sans">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-indigo-600 to-blue-600 p-6 text-white shadow-lg shadow-indigo-200">
                    ...
                    <div class="absolute -right-2 -bottom-4 opacity-20 transform rotate-12">
                        <svg class="w-24 h-24" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 13h-2v-2h2v2zm0-4h-2V7h2v6z"/></svg>
                    </div>
                </div>
                ...
                <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-rose-500 to-pink-600 p-6 text-white shadow-lg shadow-rose-200">
                    <div class="relative z-10">
                        ...
                        <div class="flex items-baseline gap-2 mt-2">
                            <h3 class="text-3xl font-bold">{{ $loans->where('is_overdue', true)->where('status', 'dipinjam')->count() }}</h3>
                            <span class="text-sm text-rose-100 bg-white/20 px-2 py-0.5 rounded text-xs animate-pulse">Overdue</span>
                        </div>
                    </div>
                    <div class="absolute -right-2 -bottom-4 opacity-20 transform rotate-12">
                ...
                <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-600 p-6 text-white shadow-lg shadow-emerald-200">
                    <div class="relative z-10">
                        ...
                        <div class="flex items-baseline gap-2 mt-2">
                            ...
                            <span class="text-sm text-emerald-100 bg-white/20 px-2 py-0.5 rounded text-xs">Selesai</span>
                        </div>
                    </div>
                    <div class="absolute -right-2 -bottom-4 opacity-20 transform rotate-12">
                        <svg class="w-24 h-24" fill="currentColor" viewBox="0 0 24 24"><path d="M9 16.2L4.8 12l-1.4 1.4L9 19 21 7l-1.4-1.4L9 16.2z"/></svg>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-col xl:flex-row gap-4 bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
                <form action="{{ route('peminjaman.index') }}" method="GET" class="flex-1 grid grid-cols-1 md:grid-cols-4 gap-3">
                    <div class="relative group md:col-span-2">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari peminjam, aset, NIP..." class="block w-full pl-10 pr-4 py-2.5 border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all shadow-sm bg-gray-50 focus:bg-white placeholder-gray-400">
                    </div>

                    <div class="md:col-span-1">
                        <select name="status" onchange="this.form.submit()" class="block w-full py-2.5 pl-3 pr-10 border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all bg-gray-50 focus:bg-white cursor-pointer">
                    ...
                    <div class="md:col-span-1">
                        @if(request()->anyFilled(['search', 'status']))
                            <a href="{{ route('peminjaman.index') }}" class="flex items-center justify-center w-full py-2.5 border border-gray-200 rounded-xl text-sm font-medium text-gray-500 hover:bg-gray-50 hover:text-gray-700 transition-colors">
                                Reset Filter
                            </a>
                        @endif
                    </div>
                ...
                <div class="flex items-center gap-2 border-t xl:border-t-0 xl:border-l border-gray-100 pt-4 xl:pt-0 xl:pl-4">
                    <a href="{{ route('peminjaman.create') }}" class="w-full xl:w-auto inline-flex items-center justify-center px-5 py-2.5 bg-indigo-600 border border-transparent rounded-xl font-semibold text-white text-sm hover:bg-indigo-700 active:bg-indigo-800 transition-all shadow-lg shadow-indigo-500/30 whitespace-nowrap">
                        ...
                        <tbody class="divide-y divide-gray-50">
                            @forelse($loans as $loan)
                                <tr class="hover:bg-gray-50/80 transition-colors duration-200 group">
                                    ...
                                    <td class="px-6 py-4">
                                        <div>
                                            <div class="font-bold text-gray-900 text-sm">{{ $loan->asset->inventory->name ?? 'Aset Dihapus' }}</div>
                                            <code class="text-xs font-mono text-indigo-600 bg-indigo-50 px-1.5 py-0.5 rounded border border-indigo-100 mt-1 inline-block">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            <div class="h-8 w-8 rounded-full bg-gray-100 flex items-center justify-center text-xs font-bold text-gray-600 border border-gray-200">
                                                {{ substr($loan->borrower_name, 0, 1) }}
                                            </div>
                                            <div>
                                                <div class="text-sm font-bold text-gray-700">{{ $loan->borrower_name }}</div>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex flex-col gap-1">
                                            <div class="text-xs text-gray-500 flex justify-between">
                                                <span>Pinjam:</span>
                                                <span class="font-medium text-gray-700">{{ \Carbon\Carbon::parse($loan->loan_date)->format('d M y') }}</span>
                                            </div>
                                            <div class="text-xs text-gray-500 flex justify-between">
                                                <span>Tenggat:</span>
                                                <span class="font-medium {{ $loan->is_overdue && $loan->status == 'dipinjam' ? 'text-rose-600 font-bold' : 'text-gray-700' }}">
                                                    {{ \Carbon\Carbon::parse($loan->return_date_plan)->format('d M y') }}
                                                </span>
                                            </div>
                                            @if($loan->status == 'kembali')
                                                <div class="text-xs text-emerald-600 font-bold mt-1 bg-emerald-50 px-1.5 py-0.5 rounded border border-emerald-100 w-fit">
                                                    Kembali: {{ \Carbon\Carbon::parse($loan->return_date_actual)->format('d M y') }}
                                                </div>
                                            @endif
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 text-center">
                                        <x-table.actions>
                                            ...
                                            @if($loan->status == 'dipinjam')
                                                <x-table.action-button onclick="openReturnModal({{ $loan->id }}, '{{ $loan->asset->inventory->name }}')" class="hover:bg-emerald-50 text-emerald-600 font-bold">
                                                    ↩️ Kembalikan Aset
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="px-6 py-20 text-center">
                                        ...
                                        <p class="text-gray-500 mt-1 mb-6 text-sm">Data sirkulasi aset akan muncul di sini.</p>
                                        <a href="{{ route('peminjaman.create') }}" class="text-indigo-600 font-bold hover:underline text-sm">
    ...
    <div id="returnModal" class="fixed inset-0 bg-gray-900/60 hidden items-center justify-center z-50 backdrop-blur-sm transition-all duration-300 opacity-0">
        <div class="bg-white p-6 rounded-2xl shadow-2xl w-full max-w-md m-4 transform scale-95 transition-all duration-300" id="modalContent">
            ...
            <form id="returnForm" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <select name="condition_after" class="w-full border-gray-200 rounded-xl text-sm focus:ring-indigo-500 focus:border-indigo-500 bg-gray-50">
                ...
                <div class="mb-6">
                    ...
                    <textarea name="return_notes" rows="2" class="w-full border-gray-200 rounded-xl text-sm focus:ring-indigo-500 focus:border-indigo-500 bg-gray-50" placeholder="Keterangan tambahan..."></textarea>
                </div>

                <div class="flex justify-end gap-3 pt-4 border-t border-gray-100">
                    <button type="button" onclick="closeModal()" class="px-4 py-2 text-gray-600 text-sm font-bold hover:bg-gray-50 rounded-xl transition-colors">Batal</button>
                    <button type="submit" class="px-5 py-2 bg-indigo-600 text-white text-sm font-bold hover:bg-indigo-700 rounded-xl shadow-lg shadow-indigo-500/30 transition-all">
                        Simpan Pengembalian
                    </button>
                </div>
            </form>
